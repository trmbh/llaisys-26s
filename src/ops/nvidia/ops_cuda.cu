...
__global__ void linear_k(std::byte*o,const std::byte*x,const std::byte*w,const std::byte*b,llaisysDataType_t t,size_t m,size_t k,size_t n){size_t z=blockIdx.x*blockDim.x+threadIdx.x;if(z<m*n){size_t r=z/n,c=z%n;float s=b==nullptr?0.f:load(b,t,c);for(size_t j=0;j<k;++j)s+=load(x,t,r*k+j)*load(w,t,c*k+j);store(o,t,z,s);}}
